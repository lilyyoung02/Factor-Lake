        ...
        for inv in factor_portfolio.investments:
            ...
            if end_price is not None:
                ...
                if entry_price is not None:
                    ...
                    log(f"{ticker} - Missing in {next_market.t}, liquidating at entry price: {entry_price}", level = "DEBUG")
...
def rebalance_portfolio(data, factors, start_year, end_year, initial_aum):
    ...
    for year in range(start_year, end_year):
        log(f"\nRebalancing Portfolio for {year} based on selected factors", level = "CRITICAL")
        #print(f"\nRebalancing Portfolio for {year} based on factors...")
        market = MarketObject(data.loc[data['Year'] == year], year)
        
        yearly_portfolio = []
        for factor in factors:
            log(f"Calculating holdings for factor: {factor.__class__.__name__}",level = "CRITICAL" )
    ...
    log(f"\nFinal Portfolio Value after {end_year}: ${aum:.2f}", level = "CRITICAL")
    log(f"Overall Growth from {start_year} to {end_year}: {overall_growth * 100:.2f}%", level = "CRITICAL")
